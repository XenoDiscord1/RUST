// main.cpp — единая точка входа — FIXED ALL ASSERTIONS
...
// ═══════════════════════════════════════════════════════
// FIX 1 — глобальные контексты + мьютекс
// ═══════════════════════════════════════════════════════
static ImGuiContext* g_GUICtx = nullptr;
static ImGuiContext* g_ESPCtx = nullptr;
static std::mutex    g_CtxMutex;

// RAII: берёт мьютекс и переключает контекст атомарно
struct CtxScope {
    std::lock_guard<std::mutex> lock;
    CtxScope(ImGuiContext* ctx, std::mutex& mtx) : lock(mtx) {
...
struct Entity {
    ...
    bool onScreen=false;
    Vec2 screen={};
    float calories=0,hydration=0,radiation=0;
    bool bleeding=false;
    std::deque<float> hpHist;
};

struct CameraData { Vec3 pos={}; Quat rot={}; float fov=60.f; };

struct ESPSettings {
    bool showPlayers=true,showNPC=true,showLoot=true,showContainers=true;
    bool showBox=true,showHP=true,showName=true,showDist=true;
    bool showSkeleton=true,showTrend=true,showEdge=true;
    float maxDist=600.f,fov=60.f;
};

struct ProcInfo { DWORD pid=0; std::string name; bool hasGameAssembly=false; };
...
struct SharedState {
    ...
    std::atomic<uint64_t> memReadTotal=0,memReadFail=0,memBytes=0;
    std::atomic<double> memReadRate=0.0,memMBs=0.0;
    std::atomic<bool> attached=false,scanning=false;
    ...
    void Log(const std::string& s) {
        ...
        char ts[32]; SYSTEMTIME t; GetLocalTime(&t);
...
float Rf32(uintptr_t a) { float v=Rpm<float>(a); return (std::isnan(v)||std::isinf(v))?0.f:v; }
...
std::string Rstr(uintptr_t addr,int maxLen=64) {
    ...
    ReadProcessMemory(G.procHandle,reinterpret_cast<LPCVOID>(addr+0x14),buf.data(),n*2,nullptr);
    std::string r;
...
DWORD FindPID(const char* name) {
    ...
    PROCESSENTRY32 pe{sizeof(pe)}; DWORD r=0;
...
uintptr_t FindModule(DWORD pid,const char* mod) {
    ...
    MODULEENTRY32 me{sizeof(me)}; uintptr_t r=0;
...
bool HasModule(DWORD pid,const char* mod) {
    ...
    MODULEENTRY32 me{sizeof(me)}; bool found=false;
    if(Module32First(snap,&me)) do {
...
void RefreshProcessList() {
    ...
    if(snap!=INVALID_HANDLE_VALUE) {
        ...
        if(Process32First(snap,&pe)) do {
            ...
            bool likely=_stricmp(pi.name.c_str(),"RustClient.exe")==0
                       ||_stricmp(pi.name.c_str(),"rust.exe")==0;
            if(likely) pi.hasGameAssembly=HasModule(pi.pid,"GameAssembly.dll");
    ...
    { std::lock_guard<std::mutex> lk(G.procMtx); G.procList=std::move(list); }
    G.listing=false;
...
void ScanEntities() {
    ...
    while(addr<end&&scanned<limit&&found.size()<64) {
        ...
        if(!VirtualQueryEx(G.procHandle,(LPCVOID)addr,&mbi,sizeof(mbi))){addr+=0x1000;continue;}
        if(mbi.State==MEM_COMMIT
           &&(mbi.Protect&(PAGE_READWRITE|PAGE_EXECUTE_READ))
           &&mbi.Type==MEM_PRIVATE
           &&mbi.RegionSize>=0x1000&&mbi.RegionSize<=0x4000000) {
            std::vector<uint8_t> buf(mbi.RegionSize); SIZE_T br=0;
            if(ReadProcessMemory(G.procHandle,mbi.BaseAddress,buf.data(),mbi.RegionSize,&br)&&br>32) {
                for(size_t i=0;i+8<=br;i+=8) {
                    ...
                    float px=Rf32(ptr+Off::PosX),py=Rf32(ptr+Off::PosY),pz=Rf32(ptr+Off::PosZ);
                    if(px<-8000.f||px>8000.f) continue;
                    if(py<-200.f||py>800.f) continue;
                    if(fabsf(px)<1.f&&fabsf(pz)<1.f) continue;
                    bool dup=false;
                    for(auto&e:found){float dx=e.pos.x-px,dz=e.pos.z-pz;if(sqrtf(dx*dx+dz*dz)<15.f){dup=true;break;}}
                    if(dup) continue;
                    Entity e; e.ptr=ptr; e.hp=hp; e.pos={px,py,pz};
                    e.type=ClassifyEntity(ptr,hp);
                    if(e.type==EType::Player){uintptr_t np=Rptr(ptr+Off::DisplayName);e.name=Rstr(np);if(e.name.empty())e.name="Player";}
                    else e.name=e.type==EType::NPC?"NPC":e.type==EType::Loot?"Loot":"Container";
                    found.push_back(e);
                }
                scanned+=br;
            }
        ...
        if(next<=addr) break;
        addr=next; G.scanPct=(float)scanned/limit*100.f;
    }
    { std::lock_guard<std::mutex> lk(G.mtx); G.entities=found; }
    G.scanning=false;
...
void GameThread() {
    ...
    while(G.running) {
        ...
        RefreshCamera();
        { std::lock_guard<std::mutex> lk(G.mtx); for(auto&e:G.entities) RefreshEntity(e); }
        DWORD now=GetTickCount(); DWORD dt=now-lastTick;
        if(dt>=1000){
...
bool AttachToPid(DWORD pid) {
    ...
    if(!h){G.Log("OpenProcess failed for PID "+std::to_string(pid));return false;}
    if(G.procHandle){CloseHandle(G.procHandle);G.procHandle=nullptr;}
    G.procHandle=h; G.procPid=pid; G.procName=NameOfPid(pid);
    G.gameBase=FindModule(pid,"GameAssembly.dll");
    G.memReadTotal=0;G.memReadFail=0;G.memBytes=0;G.memReadRate=0.0;G.memMBs=0.0;
    G.attached=true;
    G.Log("Attached: "+G.procName+" PID="+std::to_string(pid));
    if(!G.gameBase) G.Log("WARN: GameAssembly.dll not found");
...
void AttachThread() {
    ...
    for(int i=0;procs[i];++i){
        ...
        if(!pid) continue;
        G.selectedPid=pid; AttachToPid(pid); return;
    }
    G.Log("Rust not found — pick a process manually");
}

void DetachProcess() {
    G.attached=false; Sleep(20);
    { std::lock_guard<std::mutex> lk(G.mtx); G.entities.clear(); }
    if(G.procHandle){CloseHandle(G.procHandle);G.procHandle=nullptr;}
...
void DrawBox(ImDrawList*dl,float sx,float sy,float hw,float hh,ImU32 c) {
    ...
    float cs=hw*.4f; ImU32 w=IM_COL32(255,255,255,255);
...
void DrawSkeleton(ImDrawList*dl,float sx,float sy,float hw,float hh) {
    ImU32 c=IM_COL32(255,255,255,40); float hr=max(4.f,hw/3.f);
...
void DrawTrend(ImDrawList*dl,float sx,float sy,float hw,float hh,const std::deque<float>& hist) {
    ...
    std::vector<ImVec2> pts; int n=(int)hist.size();
    for(int i=0;i<n;++i)
...
void DrawEdge(ImDrawList*dl,float camX,float camZ,float ex,float ez,ImU32 c,int sw,int sh) {
    float dx=ex-camX,dz=ez-camZ,ang=atan2f(dx,dz);
...
void RenderESPFrame(int sw,int sh) {
    // FIX 2 — вызывается только внутри ESP-контекста с активным NewFrame
    ImDrawList* dl=ImGui::GetBackgroundDrawList();
    CameraData cam; std::vector<Entity> ents; ESPSettings sets;
    { std::lock_guard<std::mutex> lk(G.mtx); cam=G.camera;ents=G.entities;sets=G.settings; }
    cam.fov=sets.fov;
    for(auto&e:ents){
        if(e.lost) continue;
        bool show=false;
        ...
        Vec2 sc{}; ImU32 col=ECol(e.type);
        if(W2S(e.pos,sc,sw,sh,cam)){
            float scale=max(16.f,800.f/(dist+1.f)),hw=scale*.5f,hh=scale;
    ...
    for(auto&e:sorted){
        ...
        char buf[64]; snprintf(buf,64,"%-12s %3dHP %5.0fm",e.name.c_str(),(int)e.hp,d);
        dl->AddText({px+6,py},ECol(e.type),buf); py+=15;
    }
    DrawCrosshair(dl,sw,sh);
    DrawCompass(dl,sw);
    int alive=(int)std::count_if(ents.begin(),ents.end(),[](const Entity&e){return !e.lost&&e.hp>0&&e.type==EType::Player;});
    char hud[64];
    snprintf(hud,64,"PLAYERS %d/%d | ESC=HIDE ESP",alive,
        (int)std::count_if(ents.begin(),ents.end(),[](const Entity&e){return e.type==EType::Player;}));
...
void RenderGUI() {
    ...
    if(G.attached){
        ...
        ImGui::TextColored({.5f,.8f,1,1},"MEM READ: %.0f reads/s %.2f MB/s | total %llu fails %llu",
            rate,mbs,(unsigned long long)G.memReadTotal.load(),(unsigned long long)G.memReadFail.load());
    ...
    if(ImGui::BeginTabBar("##tabs")){
        ...
        if(ImGui::BeginTabItem("ESP")){
            ...
            ImGui::Checkbox("Players",&s.showPlayers);ImGui::Checkbox("NPC",&s.showNPC);
            ImGui::Checkbox("Loot",&s.showLoot);ImGui::Checkbox("Containers",&s.showContainers);
            ImGui::Spacing(); ImGui::Text("VISUAL");
            ImGui::Checkbox("Box",&s.showBox);ImGui::Checkbox("HP Bar",&s.showHP);
            ImGui::Checkbox("Name",&s.showName);ImGui::Checkbox("Distance",&s.showDist);
            ImGui::Checkbox("Skeleton",&s.showSkeleton);ImGui::Checkbox("HP Trend",&s.showTrend);
        ...
        if(ImGui::BeginTabItem("Players")){
            if(ImGui::BeginTable("pt",6,
                ...
                for(auto&e:G.entities){
                    ...
                    ImGui::TableSetColumnIndex(1); ImGui::TextColored({0,1,.4f,1},"%s",e.name.c_str());
                    ImGui::TableSetColumnIndex(2);
                    ...
                    ImGui::TableSetColumnIndex(4); ImGui::Text("%.0f %.0f %.0f",e.pos.x,e.pos.y,e.pos.z);
                    ImGui::TableSetColumnIndex(5);
                    const char* st=e.lost?"LOST":e.hp<=0?"DEAD":e.onScreen?"VIS":"HID";
                    ImU32 sc2=e.lost?IM_COL32(80,80,80,255):e.hp<=0?IM_COL32(255,60,60,255):
                        e.onScreen?IM_COL32(0,255,100,255):IM_COL32(200,200,200,255);
                    ImGui::TextColored(ImGui::ColorConvertU32ToFloat4(sc2),"%s",st);
                }
                ImGui::EndTable();
            }
            ImGui::EndTabItem();
        ...
        if(ImGui::BeginTabItem("Log")){
            ...
            std::lock_guard<std::mutex> lk(G.mtx);
            for(auto&s:G.log) ImGui::TextUnformatted(s.c_str());
...
struct D3DContext {
    ...
    bool Init(HWND hwnd){
        ...
        sd.OutputWindow=hwnd;sd.SampleDesc={1,0};sd.Windowed=TRUE;
        sd.SwapEffect=DXGI_SWAP_EFFECT_DISCARD;
        D3D_FEATURE_LEVEL fl;
        if(FAILED(D3D11CreateDeviceAndSwapChain(nullptr,D3D_DRIVER_TYPE_HARDWARE,
            nullptr,0,nullptr,0,D3D11_SDK_VERSION,&sd,&sc,&dev,&fl,&ctx))) return false;
...
// FIX 3 — GUIProc ВСЕГДА переключает на guiCtx перед WndProcHandler
LRESULT CALLBACK GUIProc(HWND h,UINT m,WPARAM w,LPARAM l){
    if(g_GUICtx){
        CtxScope ctx(g_GUICtx,g_CtxMutex);
...
// FIX 4 — ESPThread: явный Build() шрифтов до первого фрейма
void ESPThread(){
    ...
    g_ESPhwnd=CreateWindowExW(
        ...
        L"RustESP",L"",WS_POPUP,0,0,g_SW,g_SH,nullptr,nullptr,wc.hInstance,nullptr);
    ...
    g_ESPd3d.Init(g_ESPhwnd);

    {
        CtxScope ctx(g_ESPCtx,g_CtxMutex);
        ImGui::GetIO().IniFilename=nullptr;
        // FIX: явный Build() ПЕРЕД ImGui_ImplDX11_Init
    ...
    while(G.running){
        ...
        {
            // FIX: каждый фрейм ESP — внутри CtxScope
            CtxScope ctx(g_ESPCtx,g_CtxMutex);
...
int WINAPI WinMain(HINSTANCE hInst,HINSTANCE,LPSTR,int){
    ...
    // FIX 5 — создаём оба контекста ДО любых потоков
    g_GUICtx=ImGui::CreateContext();
    ...
    RegisterClassExW(&wc);
    g_GUIhwnd=CreateWindowExW(0,L"RustGUI",L"Company Rust Admin",
        WS_OVERLAPPEDWINDOW,100,100,560,680,nullptr,nullptr,hInst,nullptr);

    g_GUId3d.Init(g_GUIhwnd);

    // FIX: GUI контекст + явный Build() шрифтов
    {
        CtxScope ctx(g_GUICtx,g_CtxMutex);
        ImGui::GetIO().IniFilename=nullptr;
        ImGui::GetIO().Fonts->AddFontDefault();
    ...
    while(G.running){
...

        {
            ...
            RenderGUI();
            ImGui::Render();
            float clr[4]={0.05f,0.05f,0.05f,1};
